...
app.post('/creator/login', (req, res) => {
  ...
  const testerId = CREATOR_CODES[code.toUpperCase()];
  if (!testerId) return res.status(401).json({ error: 'Invalid code' });
  res.json({ testerId, code: code.toUpperCase() });
});
...
// ========================================
// MESSAGES - Get messages (user side)
// ========================================
...
// ========================================
// MESSAGES - Creator sends reply
// ========================================
app.post('/creator/reply', (req, res) => {
  const { creatorCode, userId, testerId, content, type = 'text' } = req.body;
  const authorizedTesterId = CREATOR_CODES[creatorCode?.toUpperCase()];
  if (!authorizedTesterId || authorizedTesterId !== testerId) {
...
// ========================================
// CREATOR - Get all conversations
// ========================================
app.post('/creator/conversations', (req, res) => {
  const { creatorCode } = req.body;
  const testerId = CREATOR_CODES[creatorCode?.toUpperCase()];
  if (!testerId) return res.status(401).json({ error: 'Invalid code' });

  const conversations = messageStore[testerId] || {};
  ...
  res.json({ testerId, conversations: result });
});

// ========================================
// CREATOR - Get specific conversation
// ========================================
app.post('/creator/conversation', (req, res) => {
  const { creatorCode, userId } = req.body;
  const testerId = CREATOR_CODES[creatorCode?.toUpperCase()];
  if (!testerId) return res.status(401).json({ error: 'Invalid code' });
...
4. Give clear grounded advice - no paranoia, no dramatic assumptions, no therapy jargon
...
2. Gen-Z Realism.
Use modern, natural phrasing. Light slang only if it fits the moment. Never force slang.

3. Emoji Intelligence.
Emojis are optional - not mandatory. Do NOT add an emoji unless it genuinely improves the tone. One emoji max per message cluster.

4. Emotional Texture.
Every reply must carry subtle emotional intent:
- Playful curiosity, soft challenge, calm confidence, slight mystery, light teasing, controlled temptation.
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        { role: 'system', content: systemPrompt },
        {
          role: 'user',
          content: [
            { type: 'text', text: 'Look at this conversation screenshot. Generate the next message(s) the girl should send to continue the loyalty test naturally. Output only the message(s), nothing else.' },
...
- Return ONLY valid JSON, no markdown, no extra text`;
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        { role: 'system', content: systemPrompt },
        { role: 'user', content: [{ type: 'text', text: 'Analyze these chat screenshots and return the JSON analysis. Only use what is visible.' }, ...imageContent] }
      ],
      temperature: 0.7,
      max_tokens: 1000,
    });
